...
### 二重計上の注意（下宿費用）
本アプリの下宿（`livingArrangement: 'away'`）の追加費用は、JASSO 生活費総額ではなく
**自宅外通学による追加負担（住居・光熱費相当）のみ**を教育費へ上乗せしている
（総合版の通常生活費との二重計上を避けるため）。総合版で再計算する場合も同方針を守り、
食費・娯楽費・通信費などの生活費は教育費へ加算しないこと。詳細は本アプリ
`src/lib/educationAssumptions.ts` の `UNIVERSITY_AWAY_EXTRA_YEN` 周辺コメントを参照。
